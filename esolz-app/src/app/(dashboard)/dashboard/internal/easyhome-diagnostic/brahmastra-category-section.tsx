...
export function BrahmastraCategorySection({ data, loadedRangeSuffix }: { data: ApiResponse; loadedRangeSuffix: string }) {
  ...
  return (
    <div className="space-y-6">
      {/* Single mode: top spenders / top ad sales generators */}
      {isSingle && (topSpenders.length > 0 || topAdSalesGenerators.length > 0) && (
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          {isSingle && (
            <p className="text-xs text-muted-foreground mb-3">
              Selected Period — Business Report Ordered Product Sales · SKU source range: {diagnostic.windows.afterStart} → {diagnostic.windows.afterEnd}
            </p>
          )}
          {!isBusinessReportPrimary && (
          ...
          {isSingle ? (
            <DataTable
              columns={['Portfolio', 'Ordered Product Sales', 'Units Ordered']}
              rows={[...businessReport.categoryTable]
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        <h2 className="text-sm font-bold text-foreground mb-4 flex items-center gap-2">
          ...
          {isSingle
            ? 'Category (portfolio) — Settlement net sales — selected period'
            : 'Category (portfolio) - settlement net sales delta'}
          {isBusinessReportPrimary && (
        ...
        <p className="text-xs text-muted-foreground mb-4 italic">
          {isSingle
            ? 'Settlement net sales for the selected period — reconciliation view alongside Business Report above. Settlement and Business Report can differ by date basis.'
            : (skuMapping.totalRows > 0
        ...
        {/* Bar chart of deltas only meaningful in compare mode */}
        {!isSingle && (
        ...
        {isSingle ? (
          <DataTable
            columns={['Portfolio', 'Settlement Net Sales', 'Settlement Units', 'Settlement Refunds']}
            rows={[...diagnostic.categoryTable]
              .sort((a, b) => b.afterSales - a.afterSales)
              .map(row => [
                portfolioDisplayLabel(row.portfolio),
                formatInr(row.afterSales),
      ...
      {/* SKU loser tables and diagnostic notes — only meaningful in compare mode */}
      {!isSingle && (
        <>
      ...
      {campaignDiagnostic.hasCampaignData && (
        <>
          {/* Campaign table — single mode shows selected-period columns; compare mode shows before/after */}
          <div className="bg-card border border-border rounded-xl p-5">
            <h2 className="text-sm font-bold text-foreground mb-4">
              {isSingle ? 'Campaign — selected period' : 'Campaign — before vs after'}
            </h2>
            {isSingle ? (
              <DataTable
                columns={['Campaign', 'Portfolio', 'Ad Spend', 'Ad-attributed Sales', 'ACOS', 'ROAS']}
          ...
          {/* Comparison-only campaign sections — hidden in single mode (all deltas = 0) */}
          {!isSingle && (
            <>
              <div className="bg-card border border-border rounded-xl p-5">
      ...
      {deepDiagnostic.advertisedProduct && (
        <>
          {/* Portfolio-level deep diagnostic summary */}
          <div className="bg-card border border-border rounded-xl p-5">
            <h2 className="text-sm font-bold text-foreground mb-4">
              {isSingle
                ? 'Portfolio-level deep diagnostic — SKU-attributed ad spend/sales (selected period)'
                : 'Portfolio-level deep diagnostic summary (SKU-attributed ad spend/sales)'}
            </h2>
            {isSingle ? (
              <DataTable
                columns={['Portfolio', 'Ad Spend', 'Ad-attributed Sales', 'ACOS']}
                rows={Object.values(
                  deepDiagnostic.advertisedProduct.table.reduce((acc: Record<string, { portfolio: string; afterSpend: number; afterSales: number }>, row: AdvertisedProductRow) => {
                    if (!acc[row.portfolio]) acc[row.portfolio] = { portfolio: row.portfolio, afterSpend: 0, afterSales: 0 }
                    acc[row.portfolio].afterSpend += row.afterSpend
                    acc[row.portfolio].afterSales += row.afterSales
                    return acc
                  }, {}),
                ).map(p => [
                  portfolioDisplayLabel(p.portfolio), formatInr(p.afterSpend), formatInr(p.afterSales),
          ...
          {/* Comparison-only deep report sections — hidden in single mode */}
          {!isSingle && (
            <>
              <div className="bg-card border border-border rounded-xl p-5">
      ...
      {/* Targeting sections — comparison-only, hidden in single mode */}
      {!isSingle && deepDiagnostic.targeting && (
        <>
          <div className="bg-card border border-border rounded-xl p-5">
      ...
      {deepDiagnostic.searchTerm && (
        <>
          {/* High spend, zero orders — absolute metric, useful in single and compare mode */}
          <div className="bg-card border border-border rounded-xl p-5">
            <h2 className="text-sm font-bold text-foreground mb-4">
              {isSingle
                ? 'Search terms: high spend, zero orders (selected period)'
                : 'Search terms: high spend, zero orders after 15 June'}
            </h2>
            {deepDiagnostic.searchTerm.highSpendZeroOrdersAfter.length === 0 ? (
              <p className="text-sm text-muted-foreground">None found.</p>
            ) : (
              <DataTable
                columns={['Search term', 'Campaign', isSingle ? 'Ad Spend' : 'Range B Spend', isSingle ? 'Clicks' : 'Range B Clicks']}
          ...
          {/* Comparison-only search term sections */}
          {!isSingle && (
            <>
              <div className="bg-card border border-border rounded-xl p-5">
