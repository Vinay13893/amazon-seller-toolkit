...
export async function GET(
  ...
  const { data: results, error: resultsError } = await admin
    ...
    .select('asin, buy_box_detected, price_detected, price_text, seller_name, availability_status, page_status, error_code, error_message, checked_at')
    .eq('workspace_id', member.workspace_id)
    .eq('job_id', jobId)
  ...
  if (resultsError) {
    return safeError(500, 'results_read_failed', 'Unable to read Buy Box results.')
  }
