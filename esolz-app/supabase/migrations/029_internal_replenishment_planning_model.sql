-- Additive safety migration: migration 028 was extended (commit a90ddce) with the
-- replenishment-planning schema after it may have already run in Supabase in its
-- original form. Old migrations do not re-run automatically, so this migration
-- recreates those additions idempotently to guarantee they exist in production.
...
DROP POLICY IF EXISTS "internal_fulfillment_locations: internal select"
...
DROP POLICY IF EXISTS "internal_state_zone_map: internal select"
...
DROP POLICY IF EXISTS "internal_fulfillment_sales_daily: internal select"
...
DROP POLICY IF EXISTS "internal_inventory_by_location: internal select"
